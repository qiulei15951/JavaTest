...
注意：如果让一个类称为异常类，必须要继承异常体系，因为只有称为异常体系的子类才有资格具备可抛性。
	才可以被两个关键词所操作。 throws throw




异常的分类：
1.编译时被检测异常：只要是Exception和其子类都是，除了特殊类RuntimeException体系。
		这种问题一旦出现，希望在编译时就进行检测，让这种问题有对应的处理方式。
		这样的问题都可以针对性的处理、

2.编译时不检测异常（运行时异常）：就是Exception中的RuntionmeException和其子类。
		这种问题的发生，无法让功能继续，运算无法进行，更多是因为调用者的原因导致的而或者引发内部状态的改变导致的。
		那么这种问题一般不处理，直接编译通过，在运行时，让调用者调用时的程序强制停止，让调用者对代码进行修正。



所有自定义异常时，要么继承Exception，要么继承RuntimeException
